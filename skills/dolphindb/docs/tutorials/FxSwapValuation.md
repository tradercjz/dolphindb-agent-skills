<!-- Auto-mirrored from upstream `documentation-main/tutorials/FxSwapValuation.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
**注意**：在批流一体的因子实现中，不建议使用 `ploop`/`peach` 并行加速。具体并行策略可以参考因子最佳实践中的并行计算章节。
