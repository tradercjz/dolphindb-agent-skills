<!-- Auto-mirrored from upstream `documentation-main/tutorials/datax_writer.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
* table
  ...
  + 描述：写入表的字段集合。内部结构为
    ...
    - `type`枚举值以及对应 DataX 数据类型如下。DolphinDB 的数据类型及精度，参考：：数据类型。
