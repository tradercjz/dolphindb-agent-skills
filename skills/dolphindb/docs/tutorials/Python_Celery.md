<!-- Auto-mirrored from upstream `documentation-main/tutorials/Python_Celery.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
* **教程与下载、安装：**
  ...
  有关 DolphinDB 的安装指南可以参考 DolphinDB安装使用指南。其中 Python api 的调用可以参考 Python API for DolphinDB。
...
* 本教程所介绍的是测试环境的部署，因此所部署的 DolphinDB 服务为单节点版，具体部署教程可以参考 DolphinDB 单节点部署教程；
...
* **撰写导入配置文件：**
  ...
  注：本教程涉及到的数据同步仅为历史数据的全量同步，在实际过程中如果有增量同步等的需要，在 `writer` 配置中要增加 `saveFunctionName`  和 `saveFunctionDef` 两个配置，具体用法可以参考基于 DataX 的 DolphinDB 数据导入工具。
* **执行数据导入命令：**
...
* **Celery 调用因子函数项目构建流程：**
  ...
  + `tasks.py`：该文件用来与 DolphinDB 建立 `session` 封装调用因子函数，并声明封装的函数为一个可以被 Celery 框架异步调度的任务。
    ...
    注：这里我们使用了 Python 中的数据类型的参数进行传递，其中的 Python 和 DolphinDB 的数据类型的对应关系和使用 DolphinDB 中的数据类型的传参可以参考 Python API for DolphinDB。
