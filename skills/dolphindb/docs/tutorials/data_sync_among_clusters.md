<!-- Auto-mirrored from upstream `documentation-main/tutorials/data_sync_among_clusters.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
* 异步复制：通过异步方式，将主集群复制到从集群，使主从集群数据一致，是集群异地容灾的一个解决方案。详见集群间异步复制
...
通过参数 *partition* 可以指定待恢复的分区。详细参考教程数据备份与恢复。
