<!-- Auto-mirrored from upstream `documentation-main/tutorials/matching_engine_simulator.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
在编写插件前，用户需要先学习 DolphinDB 插件开发教程，以了解插件开发的基本概念和流程。例如在 TWAP 算法交易插件中，通过 getFunctionDef 方法可以获取模拟撮合引擎的函数接口，该方法返回一个函数指针，然后通过 createEngineFunc->call(heap\_, args)来调用该函数。以下是代码示例：
