<!-- Auto-mirrored from upstream `documentation-main/tutorials/knn_iot_2.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
DolphinDB 开发了 Grafana 数据源插件，用户在 Grafana 面板上通过编写查询脚本、订阅流数据表的方式，与 DolphinDB 进行交互，实现 DolphinDB 时序数据的可视化。具体操作可参考：Grafana 数据源 。
