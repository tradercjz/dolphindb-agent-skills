<!-- Auto-mirrored from upstream `documentation-main/tutorials/kdb_to_dolphindb.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
* DolphinDB 的 SYMBOL 类型是特殊的字符串类型，相当于枚举类型。通过 SYMBOL，将字符串存储为一个整数，因此可更高效地进行数据排序。具体使用请参照数据类型。
...
对于 NULL 值的初始化、运算，以及在普通向量函数、聚合函数和高阶函数中的使用方法，参考NULL 值的操作。
...
* DolphinDB 支持包括标量、向量、数据对、矩阵、集合、字典和表在内的多种数据形式，并针对不同的使用场景做了具体优化。请参考数据形式。
...
* DolphinDB 允许用户通过取消变量或函数定义来手动释放内存。使用 `undef` 或 `<variable>=NULL`，详情请参考undef。
