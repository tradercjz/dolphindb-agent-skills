<!-- Auto-mirrored from upstream `documentation-main/tutorials/DolphinDB_TopN.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
2. 取排序结果的前 top 个元素进行计算。如果有多个具有相同值的元素无法全部进入前 top，可通过 *tiesMethod* 参数设置对这些值的选取规则。简单的说，*tiesMethod* 为 latest 时，优先选取最新的数据，为 oldest 时，优先选取最老的数据，为 all 时，选取全部数据。具体的用法规则可参考 TopN 的用户手册 TopN。
...
mTopN 可以按记录数滑动，窗口长度计算既可以按记录数，也可以按时间长度。具体滑动的规则可以参考窗口计算综述教程： window\_cal.md
