<!-- Auto-mirrored from upstream `documentation-main/tutorials/oom_settlement_2.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
* 收集 DolphinDB 配置文件内存限制信息，操作步骤如下：
  ...
  **注意**：配置文件最大内存不可超过 DolphinDB 许可证限制的内存以及不能超过服务器内存的 90%。
  ...
  如上所示为查询内存上限配置，结果为 8，则当前配置文件限制最大单次查询结果占用的内存上限为 8 GB，如果需要增大单次查询的上限，通过函数 setMemLimitOfQueryResult 进行修改。
