<!-- Auto-mirrored from upstream `documentation-main/plugins/dataxwriter/README_CN.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
* **变更数据增量同步**
  ...
  + 变更数据在数据源有不同的记录方法，比较规范的方法是通过一个变更标志和时间戳来记录，比如用 OPTYPE、 OPDATE 来记录变更的类型和时间戳，这样可以通过类似 `"where": "OPTYPE=1 and OPDATE > to_date('2020-03-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')` 条件过滤出增量数据。
    ...
    - saveFunctionName
      ...
      在 1.30.21.4 版本中，用户可通过 `saveFunctionName` 和 `saveFunctionDef` 引入 DolphinDB 的upsert! 功能以保证导入后的数据唯一性。具体配置示例参考附录。
