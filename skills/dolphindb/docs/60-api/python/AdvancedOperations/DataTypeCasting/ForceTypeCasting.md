<!-- Auto-mirrored from upstream `documentation-main/api/python/AdvancedOperations/DataTypeCasting/ForceTypeCasting.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
参考PROTOCOL\_DDB 可知，如果直接上传 `df`，此时 `cint` 列的 dtype 为 int64，仍会作为 LONG 类型上传；而由于 SYMBOL、BLOB 没有对应的类型，故直接上传的 str 型数据会被视作 STRING 类型。
