<!-- Auto-mirrored from upstream `documentation-main/modules/MarketHoliday/mkt_calendar.md`. Do not edit by hand; regenerate via `scripts/build_from_docs.py`. -->
...
我们会定期更新交易日历，更新详情可查看 交易日历更新说明。最新交易日历可通过以下方式获取：<marketHoliday.zip>。
...
`getMarketCalendar` 函数可以获取相应时间范围内的交易日。但是如若某天不是交易日，又想获得该日期前最近的一个交易日，可以使用 [`transFreq(X,rule)`](https://docs.dolphindb.cn/zh/funcs/t/transFreq.html?hl=transfreq)函数。指定 `rule` 参数为对应交易所编码，可获取对应日期的最近的交易日。以纽交所 (XNYS) 为例，获取2023年1月1日至1月6日最近的交易日历的脚本如下：
