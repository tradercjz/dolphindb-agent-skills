...
*persistenceDir*，配置参考 功能配置。流数据表在磁盘上的存储目录是
...
* 如果手动重启 server，建议调用 fflush 函数先把缓存区的数据写入磁盘再使用 `kill -15`
  命令终止进程。
...
`enableTablePersistence` 命令持久化流数据表。我们推荐使用 enableTableShareAndPersistence 函数将共享和持久化组织成原子性操作。
