...
*textChunkDS* 函数将文本文件划分为多个小文件数据源，再通过 mr 函数写入到数据库中。与直接把大文本文件加载到数据库对比，这种方法占用的内存更少。

当 DolphinDB
加载数据文件时，会进行随机抽样，并基于样本决定每列的数据类型。这个方法不一定每次都能准确决定各列的数据类型。因此我们建议，在加载数据前，使用 extractTextSchema 函数查看 DolphinDB
...
列中指定数据文件中日期或时间的格式（用字符串表示），如 "MM/dd/yyyy"。如何表示日期和时间格式请参考 日期和时间的调整及格式。
...
**schema** 表对象，用于指定各字段的数据类型。具体请参考 loadText
的 *schema* 参数。
