...
* 解析类型：
  ...
  + 当 DolphinDB
    ...
    extractTextSchema 函数查看 DolphinDB 识别每列的数据类型。
  ...
  + 如果 DolphinDB 识别的数据类型不符合预期，可以在 schema 的 type 列中指定数据类型。对于日期列或时间列，如果
    ...
    列中指定数据文件中日期或时间的格式（用字符串表示），如 "MM/dd/yyyy"。如何表示日期和时间格式请参考 ParsingandFormatofTemporalVariables。
...
DolphinDB 时间类型不兼容，建议导入时先指定为字符串类型，再通过 temporalParse 函数进行转换。
